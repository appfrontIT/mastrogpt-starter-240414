...
def delete(args):
    ...
    if not name:
        return {"statusCode": 400}
    resp = requests.delete(f"https://nuvolaris.dev/api/v1/namespaces/gporchia/packages/{name}?force=true", auth=HTTPBasicAuth(SPLIT[0], SPLIT[1]))
...
def share(args):
    ...
    user = requests.get(f'https://nuvolaris.dev/api/v1/web/gporchia/base/user/find?id={id}')
