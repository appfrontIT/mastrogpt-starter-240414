...
#--timeout 600000
#--annotation url https://walkiria.cloud/api/v1/namespaces/{os.environ['__OW_NAMESPACE']}/actions/db/db_store_init
...
def main(args):
    ...
    text = crawl(url)
    format_obj = requests.post(f'https://walkiria.cloud/api/v1/namespaces/{os.environ['__OW_NAMESPACE']}/actions/utility/md_to_json?blocking=true', auth=HTTPBasicAuth(OW_API_SPLIT[0], OW_API_SPLIT[1]), json={
        "md": format
    })
    obj = format_obj.json()
    ...
    """
    action = requests.post(f'https://walkiria.cloud/api/v1/namespaces/{os.environ['__OW_NAMESPACE']}/actions/walkiria/generate?blocking=true',
    ...
    while True:
        ...
        if not lines:
            break
        requests.post(f'https://walkiria.cloud/api/v1/namespaces/{os.environ['__OW_NAMESPACE']}/actions/db/db_store_exec',
                    auth=HTTPBasicAuth(OW_API_SPLIT[0], OW_API_SPLIT[1]),
