...
def gathering_data_from_url(url = None):
    if url != None:
        response = requests.post(f'https://walkiria.cloud/api/v1/web/mcipolla/utility/single_page_scrape', json={'url': url})
        if response.ok:
...
def generate_graph(name, request, collection = '', text = '', url = ''):
    ...
    if collection != '':
        ...
        if collection in collection_list:
            ...
            return {"collection provided does not exists"}
        data = db_coll.find({}, {'_id': 0}).limit(10)
        messages.extend([{'role': 'user', 'content': f"""You must call this endpoint to fetch data from database, making the aggregate and call this API: ('https://walkiria.cloud/api/v1/web/mcipolla/db/mongo/{collection}/aggregate, json={{'pipeline': [...]}}).
