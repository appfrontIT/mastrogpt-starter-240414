...
def domain_scraping(url = '', embedding = False):
    if url == '':
        return "No url provided"
    resp = requests.post("https://walkiria.cloud/api/v1/namespaces/mcipolla/actions/utility/apify_scraper",
                        auth=HTTPBasicAuth(config.OW_API_SPLIT[0], config.OW_API_SPLIT[1]),
...
def html_gen(args):
    ...
    if action_array != "":
        ...
        query = description
    html = requests.post("https://walkiria.cloud/api/v1/web/gporchia/html_gen/bot",
...
def update_action(function = False, fix = False):
    if function and fix:
        ...
        response = requests.post('https://walkiria.cloud/api/v1/namespaces/mcipolla/actions/walkiria/generate?blocking=true',
                            auth=HTTPBasicAuth(config.OW_API_SPLIT[0], config.OW_API_SPLIT[1]),
...
def store_text_in_specific_format(url, collection, format):
    return requests.post('https://walkiria.cloud/api/v1/namespaces/mcipolla/actions/db/db_store_init', auth=HTTPBasicAuth(config.OW_API_SPLIT[0], config.OW_API_SPLIT[1]), json={
        "url": url,
...
def delete_action(name = False, package = False):
    ...
    delete = requests.delete('https://walkiria.cloud/api/v1/web/gporchia/base/openAPI/delete',
