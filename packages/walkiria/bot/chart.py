...
def grapher(type = None, request = None, name = None, description = None):
    ...
    if collection:
        data = requests.get(f'https://walkiria.cloud/api/v1/web/mcipolla/db/mongo/{collection}/find_many',
                            headers={'Authorization': 'Bearer ' + config.session_user[ 'JWT']}).json()
    else:
        data = type.get('data', False)
    graph = requests.post(
        f'https://walkiria.cloud/api/v1/namespaces/mcipolla/actions/grapher/create?blocking=true',
