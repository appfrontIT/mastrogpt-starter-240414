...
def grapher(type = None, request = None, name = None, description = None):
    ...
    if collection:
        data = requests.get(f'https://nuvolaris.dev/api/v1/web/gporchia/db/mongo/mastrogpt/{collection}/find_many',
                            headers={'Authorization': 'Bearer ' + config.session_user[ 'JWT']}).json()
    else:
        data = type.get('data', False)
    graph = requests.post(
        'https://nuvolaris.dev/api/v1/namespaces/gporchia/actions/grapher/create?blocking=true',
