...
#--timeout 300000
#--annotation url https://walkiria.cloud/api/v1/namespaces/{os.environ['__OW_NAMESPACE']}/actions/walkiria/generate
...
Mandatory:
    ...
    3 - If you have to store data inside a database you MUST use the following action: https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/db/mongo/{collection}/{operation}.
        ...
        ```python
            ...
            def main(args):
                ...
                if path == '/create':
                    response = request.post("https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/db/mongo/books/add", headers = {"Authorization": token}, json={"data": {"title": args.get("title"), "author": args.get("author"), "pages": args.get("pages"), "year": args.get("year")}})
                    return {"statusCode": response.status_code, "body": response.json()}

                # delete an element by id
                elif path == '/delete':
                    response = request.delete("https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/db/mongo/books/delete?id=" + args.get('id'), headers = {"Authorization": token},)
                    return {"statusCode": response.status_code, "body": response.json()}

                # update an element by id using 'data'
                elif path == '/update':
                    response = request.put("https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/db/mongo/books/update?id=" + args.get('id'), headers = {"Authorization": token}, json={"data": {"title": args.get("title"), "author": args.get("author"), "pages": args.get("pages"), "year": args.get("year")}})
                    return {"statusCode": response.status_code, "body": response.json()}

                # find single element matching the filter
                elif path == '/find_one':
                    response = request.get("https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/db/mongo/books/find_one?title=" + args.get('title') + "&author=" args.get('author'), headers = {"Authorization": token},)
                    return {"statusCode": response.status_code, "body": response.json()}

                # find all elements matching the filter
                elif path == '/find_many':
                    response = request.get("https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/db/mongo/books/find_many", headers = {"Authorization": token},)
                    return {"statusCode": response.status_code, "body": response.json()}
...
def deploy_action(name, function, description, language, returns = None):
    ...
    package_up = requests.get(f"https://walkiria.cloud/api/v1/namespaces/{os.environ['__OW_NAMESPACE']}/packages/{package}", auth=HTTPBasicAuth(OW_API_SPLIT[0], OW_API_SPLIT[1]))
    ...
    deploy = requests.put(f'https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/base/action/add',
    ...
    # requests.post("https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/db/code_editor/add", json={'editor': editor}, headers={'cookie': f"appfront-sess-cookie={SESSION_USER['cookie']}"})
    return f"""url: https://walkiria.cloud/api/v1/web/{os.environ['__OW_NAMESPACE']}/{package}/{name}\ndescription: {description}\nfunction: {function}"""
